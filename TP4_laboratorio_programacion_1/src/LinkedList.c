		...
		nodeIndex<size)
	{
		...
		if(pNodeAux != NULL)
		{
			for(i=0;i<=nodeIndex;i++)
			{
				...
				else if(pNodeAux->pNextNode != NULL)
				{
...
{
    int returnAux = -1;
    int sizeLl=ll_len(this);

    if(this!=NULL)
    {
		addNode(this,sizeLl,pElement);
		returnAux=0;
    }
...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this != NULL && index >= 0 && index <= ll_len(this))
    {
        pNodoDel = getNode(this,index);
        if(pNodoDel != NULL)
        {
            if(index == 0)
            {
                if(this->pFirstNode != NULL) this->pFirstNode = pNodoDel->pNextNode;
            }
            else
            {
                pNodoAnterior = getNode(this,index-1);
                pNodoAnterior->pNextNode = pNodoDel->pNextNode;
            }
            free(pNodoDel);
            this->size--;
            returnAux = 0;
...
int ll_containsAll(LinkedList* this,LinkedList* this2)
{
	...
	int i;
	int cont=0;
	int sizeThisDos=ll_len(this2);
    if(this2!=NULL && this!=NULL)
    {
		for(i=0;i<sizeThisDos;i++)
		{
